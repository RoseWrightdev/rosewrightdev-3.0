  ...
  return (
    <span>
      <div className={'flex justify-between	' + styles.tabs}>
        {tabs.map((tab, i) =>
          <button className={'bg-white dark:bg-black-900 dark:hover:text-white dark:text-white text-black-900 hover:bg-gray-800 dark:hover:bg-black-600 disabled:hover:border-l-black-500 ' + styles.tabButton} key={i} id={tab.id} disabled={currentTab === `${tab.id}`} onClick={(handleTabClick)}>{tab.tabTitle}</button>
        )}
      </div>
      <div>
        {tabs.map((tab, i) =>
